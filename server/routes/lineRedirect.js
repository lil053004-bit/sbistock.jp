...
function validateRedirectUrl(urlString) {
  if (!isValidUrl(urlString)) {
    return {
      valid: false,
      error: 'Invalid URL. Only HTTPS URLs from allowed domains (line.me, lin.ee, liff.line.me) are permitted.'
    };
  }
  return { valid: true };
}
...
router.post('/', authMiddleware, (req, res) => {
  ...
  try {
    ...
    if (!redirect_url) {
      return res.status(400).json({ success: false, error: 'Redirect URL is required' });
    }

    const validation = validateRedirectUrl(redirect_url);
    if (!validation.valid) {
